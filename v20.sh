...
data_ip="https://raw.githubusercontent.com/rajaganjil93/ganjil2/main/ip"
checking_sc() {
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/rajaganjil93/ganjil2/main/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/rajaganjil93/ganjil2/main/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/rajaganjil93/ganjil2/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/rajaganjil93/ganjil2/main/"
...
restart_system() {
    USRSC=$(wget -qO- https://raw.githubusercontent.com/rajaganjil93/ganjil2/main/ip | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://raw.githubusercontent.com/rajaganjil93/ganjil2/main/ip | grep $ipsaya | awk '{print $3}')
    TIMEZONE=$(printf '%(%H:%M:%S)T')
    TEXT="
<code>────────────────────</code>
<b>⚡AUTOSCRIPT PREMIUM V.02 LTS⚡</b>
<code>────────────────────</code>
...
bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install -u www-data --version $latest_version
    ...
    #wget -O /usr/local/bin/xray "${REPO}julak/xray.linux.64bit" >/dev/null 2>&1
...
wget -O https://sc1.bagusvpn.me/limit/limit.sh && chmod +x limit.sh && ./limit.sh
cd
...
# // Installing UDPGW
wget -O /usr/bin/badvpn "${REPO}waluh/badvpn" >/dev/null 2>&1
chmod +x /usr/bin/badvpn > /dev/null 2>&1
wget -q -O /etc/systemd/system/badvpn1.service "${REPO}waluh/badvpn1.service" >/dev/null 2>&1
wget -q -O /etc/systemd/system/badvpn2.service "${REPO}waluh/badvpn2.service" >/dev/null 2>&1
wget -q -O /etc/systemd/system/badvpn3.service "${REPO}waluh/badvpn3.service" >/dev/null 2>&1
...
print_install "Installing the SlowDNS Server module"
    wget -q -O /tmp/nameserver "${REPO}baku/nameserver" >/dev/null 2>&1
...
wget -q -O /etc/ssh/sshd_config "${REPO}linak/sshd" >/dev/null 2>&1
...
wget -q -O /etc/default/dropbear "${REPO}linak/dropbear.conf"
...
# Ganti Banner
wget -O /etc/kyt.txt "${REPO}linak/issue.net"
...
print_install "Installing WebSocket Proxy"
    wget -O /usr/bin/ws "${REPO}bantur/ws.py" >/dev/null 2>&1
